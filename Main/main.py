...
def main(DeviceNum):
    ...
    #DeviceNum = 'c5bac654'
    NeedSwipe = ['CLB0218414001154','DLQ0216824000142','E4J4C17405011422','APU0216530000778','APU0216408028484','APU0216111008105']
    try:    
        DbContext = DbHelper()        
        DeviceType = 0
        ...
        print('\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~o(╥﹏╥)o 设备连接异常 o(╥﹏╥)o~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        DbContext.AddLog(DeviceNum,3,"设备["+ DeviceNum +"]连接异常：" + repr(e).replace("'",""))        
    else:
        BackHomeStatus = BackHomePage(poco,DbContext,DeviceNum,device) #返回首页
        if BackHomeStatus:
            ...
            if mode == 1:
                ...
                SearchCatchStore('7ad427b0305a11ea81c9005056c00008',poco,device,DeviceNum,DeviceType)
            if(len(taskList)==1):
                #更新任务为运行中
                DbContext.UpdateTaskStatus(int(taskList[0]['TaskId']),1,0,mode)
            while IsRunning:
                BackHomeStatus = BackHomePage(poco,DbContext,DeviceNum,device) #返回首页
                if BackHomeStatus:
                    for task in taskList:
                        AllPosition = [{'RepresentativeAdress':task['RepresentativeAdress'],'Genhash':task['Genhash']}]#AddressList(task['TargetCity']) #获取未抓取的坐标点    
                        taskId = task['TaskId']
                        ...
                        #返回值需要写进队列
                        currentTaskResult,IsEmergencyStop = StartCapture(poco,AllPosition,DeviceType,task['TargetCity'],DeviceNum,cityCode,device)#抓取数据
                        #没有紧急置停的情况下才完成后续的更新
                        if not IsEmergencyStop:
                            EndTime = datetime.datetime.now()
                            DbContext = DbHelper()
                            DbContext.AddLog(DeviceNum,2,'设备['+ DeviceNum +']本次抓取['+ AllPosition[0]['RepresentativeAdress'] +'] ['+ str(len(currentTaskResult)) +'] 家店,耗时：' + str(((EndTime - StartTime).seconds)/60))                                              
                            #更新任务为完成
                            DbContext.UpdateTaskStatus(int(taskId),2,len(currentTaskResult),mode)   
                            #将任务的执行结果回写到队列
                            if mode != 3:
                                Produce = SendMessage()
                                result = Produce.sendMessage(taskId,currentTaskResult)
                                if result:
                                    #将任务状态改为已回写队列
                                    DbContext.UpdateTaskStatus(int(taskId),3,len(currentTaskResult),mode)   
                        else:
                            break
                    mode = DbContext.GetDeviceRunningMode(DeviceNum)
                    if len(taskList) > 0:
                    ...
                    if len(taskList) > 0:
                        ...
                        if(((newTime - lateTime).seconds)/60>60 or (newTime - lateTime).days>0):
                            print("最后执行的任务执行了"+str((newTime - lateTime).days)+"天"+str(((newTime - lateTime).seconds)/60)+"分钟还没完成")
                            #把意外中断的任务重启
                            DbContext.rebootTask()
                        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~O(∩_∩)O 抽取一条任务 O(∩_∩)O~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
                    else:
                        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~O(∩_∩)O 任务执行完毕 O(∩_∩)O~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
