...
#原价
def GetOriginPrice(productPrice,poco) -> float:
    _productBeforePrice = -1
    switchNum = 0
    while True:
        try:
            if productPrice.child(name = 'com.sankuai.meituan.takeoutnew:id/txt_stickyfoodList_adapter_food_original_price_fix').exists():
                _productBeforePriceText = productPrice.child(name = 'com.sankuai.meituan.takeoutnew:id/txt_stickyfoodList_adapter_food_original_price_fix').get_text()
...
def GetDiscount(product,poco) -> str:
    ...
    while True:
        try:
            if product.child(name = 'android.widget.LinearLayout').child(name = 'com.sankuai.meituan.takeoutnew:id/txt_promotion_info').exists():                                    
                _productDiscount = product.child(name = 'android.widget.LinearLayout').child(name = 'com.sankuai.meituan.takeoutnew:id/txt_promotion_info').get_text()
                break
            else:
                poco.swipe([0.5,0.7],[0.5,0.65],duration = 0.3)
                switchNum += 1
...
def GetSale(product,poco) -> int:
    ...
    while True:
        try:
            if product.child(name = 'com.sankuai.meituan.takeoutnew:id/tv_stickyfood_sold_count').exists():
                _productSalesVolumeStr = product.child(name = 'com.sankuai.meituan.takeoutnew:id/tv_stickyfood_sold_count').get_text()
