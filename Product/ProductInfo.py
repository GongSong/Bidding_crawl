        ...
        try:             
            ...
            if len(CurrentClassfiy) > 0:                                    
                ...
                for product in CurrentClassfiy:   
                    try:         
                        #商品信息                        
                        product = product.child(name = 'com.sankuai.meituan.takeoutnew:id/ll_stickyfoodList_adapter_food_food')
                        #名称
                        ...
                        if _productName in AllProductInfoList:
                            ...
                            #原价
                            if CategaryNum < 2: 
                                _productBeforePrice = GetOriginPrice(_productPrice,poco)                                    
                            else:
                        ...
                        if('UIObjectProxy' in repr(e)):                            
                            ...
                            continue                            
            #判断是否滑动到底部
            Isbottom = poco("com.sankuai.meituan.takeoutnew:id/noMoreView").wait(waitTime).exists()    
            if Isbottom:
                BottomNum += 1
                if BottomNum == 1:
                    print('~~~~~~~~~~~~~~~~~~~~到达底部~~~~~~~~~~~~~~~~~~~~')
                ...
                CurrentProductNum = len(AllProductInfoList)
                productNum.append(CurrentProductNum)
                #统计每次这个数量出现的频率,因为有的时候滑动到底部不会有到底的提示,所以当商品数量多次滑动不再增加的时候
                #就认为商品抓取完毕
                CurrentProductNumShowNums = 0
                for num in productNum:
                    if num == CurrentProductNum:
                        CurrentProductNumShowNums += 1
                #滑动10次依旧没有商品新增,则认为到底了
                if CurrentProductNumShowNums > 10:
                    print('~~~~~~~~~~~~~~~~~程序判定滑动到了底部~~~~~~~~~~~~~~~')
                    break
                else:
                    poco.swipe([0.5,0.7],[0.5,0.4],duration = 0.3)
