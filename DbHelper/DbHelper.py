...
class DbHelper(DbBase):    
    ...
    def InsertShop(self,shopName,wmPoiScore,address,SellNum,AnchorPoint,Genhash,city,cityCode):
        ...
        try:            
            ...
            if 'Duplicate' in repr(e):
                #如果之前存在那就更新销量和评分(定位点可能会存在多个，要存多条)
                sql = '''
                    update shop
                    set wmPoiScore = '%s',
                    SellNum = %d,
                    AnchorPoint = Concat('%s',if(AnchorPoint is null,'',AnchorPoint)),
                    Genhash = Concat('%s',if(Genhash is null,'',Genhash))
                    where shopName = '%s' and address = '%s' 
                '''
                sql = sql % (wmPoiScore,int(SellNum),AnchorPoint+';',Genhash+';',shopName,address)
                cursors = self._conn.cursor()
    ...
    #根据门店名字和地理散列查找门店信息
    def GetStoreInfo(self,shopName,wmPoiScore,SellNum,Genhash):
        sql = "select count(*) as nums from shop where shopName = '" + shopName + "' and Genhash like '%" + Genhash + "%' and address not like '%" + FaileAddress + "%'" 
        cursors = self._conn.cursor()
        cursors.execute(sql)
        result = cursors.fetchall()        
        nums = result[0]['nums']        
        if int(nums) == 0 or int(nums) > 1:#不存在继续爬门店信息；存在多条，说明同一定位点可能有名称相同但是地址不用，这时候继续爬            
            return True,'',''
        elif int(nums) == 1:#只有唯一的一条说明之前爬过不需要爬地址，直接更新销量等信息
            sql = "update shop set wmPoiScore = '" + wmPoiScore + "',SellNum = " + SellNum + " where shopName = '" + shopName + "' and Genhash like '%" + Genhash + "%'"            
            try:
                cursors = self._conn.cursor()
                result = cursors.execute(sql)                
                self._conn.commit()     
                ...
                #获取这家店的地址
                sql = "select mtWmPoiId,address from shop where shopName = '" + shopName + "' and Genhash like '%" + Genhash + "%' and address not like '%" + FaileAddress + "%'"        
                cursors.execute(sql)
                result = cursors.fetchall()
